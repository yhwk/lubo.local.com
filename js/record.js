$(document).ready(function(){
   ...
   $(".ftp-rd").click(function(){
	 var y=$(this).val();
     if(y==1){$("input[type=text]").attr("disabled","disabled");$(".change,input[type=text]").css({"color":"#ddd"});}
	 if(y==0){$("input[type=text]").removeAttr("disabled");$(".change,input[type=text]").css({"color":"#000"});}
   });
  ...
  +
   ...
   $("#met-det-btn").click(function(){
    ...
    $(window).load(function(){
		...
		if(h<350){$(".footer").css("margin-top",h-150);}
		
		banner=new Banner();
		bannerCtrl(banner);
		
		var ut = getCookie("type");
		if(ut==0){$(".logout-header").find("span").first().html("管理员")}
		else if(ut==1){$(".logout-header").find("span").first().html("普通用户")}
		
		var ut = getCookie("type");
		if(ut==1){
			$(".siderbar-menu").find("li").get(0);
		}	
	})
  ...
  function getCookie(cookie_name)   
   ...
   //.serialize()获取的值转成json
	...
	$("#login-btn").click(function(){
        ...
        $.post(Rurl, {"method":"27","type":"1","sub_type":"0","my_id":"","password":pwd},function(data, status){
		   if(status == "sucess"){			   
			  // if(){
	  ...
	  if(type==0){window.location.href="basicSet.html"}
	  else if(type==1){window.location.href="courseLive.html"}	   	
	})
	//
	...
	$("#delet-btn").click(function(){
		var Hurl="/back_end/course_dem.php";
		var data=[];
	...
	//交互？无接口
		 ...
		 //交互辅流开关？
	     $("#send-flow").click(function(){
			 if($(this).prop('checked')==true){
				 $.post(Rurl,{"method":"","is_start":"1"})
			 }
		     else{
				 $.post(Rurl,{"method":"","is_start":"0"})
				 }
		 })	
		 //交互会议？
	     $(".met-det-btn").click(function(){
	      $.get("/back_end/status_ctr",function(data){
		      $(".met-chs-box").html("<div class='radio met-radio'><label><input type='radio' name='metChs'   value='"+value+" checked> 会议ID "+id+"   <span>"+ courseName+"<span></label></div>")
	      })
		 })
	   
	   //下载
	   $(".download").click(function(){
		   $.post("/back_end/status_dem")
	   })		 
})

//状态显示？无接口
function getTab(){
	  $.post("/back_end/",{"method":"","":""},function(data){
...
//视频直播
 function getLive(){
     var live;
	 /*$.get("/back_end/course_live",function(data){
	       	  live=data;			 
			  return live;
	  })*/
	 var a={"url":"rtmp://live.hkstv.hk.lxdns.com/live/hks","PLAN_NAME":"张三测试","START_TIME":"2016-3-20 12:00","LIVE_HEAD":"无主题","LIVE_MEMO":"无详情"};return a;
 }
